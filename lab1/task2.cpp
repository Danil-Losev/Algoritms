...
int *sumOfColumnArray(int **fArray, int sizeLine, int sizeColumn);
...
int main() {
  ...
  sArray = sumOfColumnArray(Array, SIZE_LINE, SIZE_COLUMN);
  numberOfBiggestLine = biggestLineOfArray(sArray, SIZE_LINE);
  std::cout << "LINE WITH THE BIGGEST SUM: " << numberOfBiggestLine
            << " + 1 = " << numberOfBiggestLine + 1;
  for (int i = 0; i < SIZE_LINE; i++) {
    delete[] Array[i];
  }
  delete[] Array;
